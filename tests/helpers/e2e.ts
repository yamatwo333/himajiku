...
export const E2E_LINKED_GROUP_ID = "group-linked";
...
export async function stubClipboard(page: Page, options?: { disableShare?: boolean }) {
  await page.addInitScript(({ disableShare }) => {
  ...
  }, { disableShare: options?.disableShare ?? false });
}

export async function readStubClipboard(page: Page) {
